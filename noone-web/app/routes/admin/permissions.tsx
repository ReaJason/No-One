...
import { deletePermission, getPermissions, loadPermissionSearchParams } from "@/api/permission-api";
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  const permissionId = Number(String(formData.get("permissionId") ?? ""));
  if (!Number.isFinite(permissionId)) {
    return { errors: { general: "Invalid permission ID" } };
  }
  ...
  try {
    ...
    return { errors: { general: error?.message || "Failed to delete permission" } };
  }
}
